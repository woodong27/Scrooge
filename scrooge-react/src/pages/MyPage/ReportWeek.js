    ...
    <div>
      ...
      <div className={styles.weekDays}>
        <button onClick={() => handleWeekBtn(-7)}>
          <img src={`${process.env.PUBLIC_URL}/images/left.svg`} alt="이전" />
        </button>
        <h3>
          {formatDate(startDate, "label")} ~ {formatDate(endDate, "label")}
        </h3>
        <button onClick={() => handleWeekBtn(+7)}>
          <img src={`${process.env.PUBLIC_URL}/images/right.svg`} alt="다음" />
        </button>
      </div>
      ...
      <div className={styles.reportContent}>
        <div className={styles.weekAvg}>
          <div>평균소비금액📊</div>
          <div>
            <b>{Math.floor(resultsSummary().avg).toLocaleString()}</b> 원
          </div>
        </div>
        <div className={styles.weekMax}>
          <div>이번주 과소비💸</div>
          <div>
            <b>{resultsSummary().max.toLocaleString()}</b> 원
          </div>
        </div>
      </div>
        ...
        {showGraph ? (
          ...
          >
            <Bar
              ...
              fill="#A2D660"
              barSize={35}
              shape={<Rectangle radius={[15, 15, 0, 0]} />}
            >
              <LabelList
                dataKey="amount"
                position="top"
